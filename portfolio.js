...
if (window.matchMedia("(min-width: 769px)").matches) {
    ...
    // Function to handle kvadrat elements entering the viewport
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const kvadratObserver = new IntersectionObserver((entries) => {
            ...
            threshold: [0], // Trigger when any part of the element is visible
            rootMargin: '0px' // Optional: Adjust based on your layout
        });
    
        // Observe each kvadrat element
        kvadratElements.forEach(element => {
    ...
    const scrollbutton1 = document.querySelector('.dahome')
    ...
    const scrollbutton2 = document.querySelector('.pro')
    ...
    const scrollbutton3 = document.querySelector('.abouter')
    ...
    const scrollbutton4 = document.querySelector('.contacter');
    
    if (scrollbutton4) { // Check if the element exists
        scrollbutton4.addEventListener('click', () => {
            // Example with fixed 'top' value
            window.scroll({
                top: 1900, // Ensure the page height supports this value
                behavior: "smooth"
            });
        });
    }
